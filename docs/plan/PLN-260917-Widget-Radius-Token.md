---
document_id: WIDGET-RADIUS-FN-1.1.0
version: 1.1.0
status: Implemented
created: 2026-09-17
updated: 2026-09-17
...
기대합니다. 그래서 비-핀 라운딩 호출부 53곳 중 52곳이 누락됐습니다. 수정은 의존 방향을
...
| `--ivy-radius` 하나 + 컴포넌트별 수동 CSS | ❌ 현재 상태 — 52곳 누락 |
...
- **Rule 4**: 커스텀 디자인이 없으면 변수는 미설정이고 Tailwind 폴백(6/8/12/16px)이
  적용 — 오늘의 기본 위젯과 동일.
...
- **모듈**: `apps/widget/src/lib/theme.ts` — `THEMED_PROPERTIES`에 파생 변수 4개 추가.
...
패널 기준 **47개소**(전체 소스 53개소 중 storefront 6개소는 Rule 5로 제외).
기계적 치환은 시작점이고, 41개 `rounded-lg`는 개별 검토 — 썸네일·조밀한 칩은 `st-sm`이
나을 수 있음.
...
- **구현**: `scripts/check-radius-tokens.mjs` + `npm run radius:check` + CI 스텝
  ...
  패널 컴포넌트에 `rounded-(sm|md|lg|xl|2xl|3xl)`가 나타나면 실패하고 **대체 토큰을
  지목**. `rounded-full`·`rounded-none`·`rounded-bl-sm`·storefront는 허용.
...
| ID | 요구사항 | 우선순위 |
|----|-------------|----------|
| FR-001 | 테마 레이어가 4단 파생 스케일 발행 | P0 |
...
| T-003 | 47개소 이행 + 개별 검토 (FN-003) | T-002 | 1.5d | ✅ |
